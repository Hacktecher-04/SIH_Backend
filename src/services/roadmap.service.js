...
    const generatedContent = await generateData(topicTitle); // Renamed variable for clarity
    if (!generatedContent) {
...
          // --- FIX HERE: Changed .youtube to .videos ---
          t.videoResources = generatedContent.videos.map(item => ({
            title: item.title,
            url: item.url,
            type: item.type
          }));
